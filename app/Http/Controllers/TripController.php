...
class TripController extends Controller
{
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->type == 'company')
        {
            ...
            if(isset($setting['twilio_trip_create_notification']) && $setting['twilio_trip_create_notification'] ==1)
            {
                 $msg = $request->purpose_of_visit.__(" is created to visit").' '.$request->place_of_visit.' '.__("from").' '.$request->start_date.__(" to").' '.$request->end_date.'.';
