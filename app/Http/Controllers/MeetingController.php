...
class MeetingController extends Controller
{
    ...
    public function calendar(Request $request)
    {
        if(\Auth::user()->type == 'company' || \Auth::user()->type == 'employee')
        {
            ...
            $meeting_current_month =  Meeting::whereMonth('date', date('m')) ->whereYear('date', date('Y')) ->get(['title','date']);
            $defualtView         = new UserDefualtView();
            $defualtView->route  = \Request::route()->getName();
            $defualtView->module = 'meeting';
            $defualtView->view   = 'calendar';
            User::userDefualtView($defualtView);
            return view('meeting.calendar', compact( 'arrMeeting','meeting_current_month','transdate'));
        }
    ...
    public function export()
    {
        ...
        $data = Excel::download(new MeetingExport(), $name . '.xlsx'); ob_end_clean();
