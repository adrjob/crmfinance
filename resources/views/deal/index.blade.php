...
@section('action-btn')
    ...
    @if(\Auth::user()->type=='company')
        @if($pipeline)
            <div class="btn-group">
                <lable class="btn btn-sm btn-primary btn-icon m-1" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    {{$pipeline->name}}
            </lable>
    ...
    <a href="#" class="btn btn-sm btn-primary btn-icon m-1" data-bs-toggle="modal"
        data-bs-target="#exampleModal" data-url="{{ route('deal.create') }}"
        data-bs-whatever="{{__('Create New Deal')}}"
        data-bs-original-title="{{__('Create New Deal')}}">
            ...
            <div class="row align-items-center justify-content-between">
                <div class="col-auto mb-3 mb-sm-0">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="ms-3">
                            <!-- <small class="text-muted">{{ __('Statistics') }}</small> -->
                            <h6 class="m-0">{{__('This Week Total Deals')}}</h6>
                                            ...
                                            <div class="pt-3 ps-3">
                                                @if($labels)
                                                    @foreach($labels as $label)
                                                        <span class="badge bg-{{$label->color}} p-1 px-3 rounded">{{$label->name}}</span>
                                                    @endforeach
                                                @endif
                                                <div class="card-header border-0 pb-0 position-relative">
                                                    <h5>
                                                        ...
                                                        data-bs-toggle="tooltip"  title data-bs-original-title="{{__('Deal Detail')}}" >
                                                        {{$deal->name}}</a></h5>
                                                        @if(\Auth::user()->type=='company')
                                                            <div class="card-header-right">
                                                                <div class="btn-group card-option">
                                                                    ...
                                                                    <div class="dropdown-menu dropdown-menu-end">
                                                                        @if(\Auth::user()->type=='company')
                                                                                <a href="#!" class="dropdown-item" data-size="lg" data-url="{{ route('deal.edit',$deal->id) }}"
                                                                                    data-bs-toggle="modal" data-bs-target="#exampleModal"  data-bs-whatever="{{__('Edit Deal')}}">
                                                                            ...
                                                                            @if(\Auth::user()->type=='company')
                                                                                <span class="">
                                                                                    ...
                                                                                    <a href="#!" class="dropdown-item show_confirm ">
                                                                                        <i class="ti ti-trash text-black"></i>{{ __('Delete') }}
                                                                                    </a>
                                                                                    {!! Form::close() !!}
                                                                                </span>
                                                                            @endif
                                                ...
                                                <div class="card-body">
                                                    <p class="text-muted text-sm">{{count($deal->tasks)}}/{{count($deal->complete_tasks)}} {{ __('Task') }}</p>                                                <div class="d-flex align-items-center justify-content-between">
                                                        <ul class="list-inline mb-0">
                                                            <li class="list-inline-item d-inline-flex align-items-center"><i
                                                                    class="f-16 text-primary ti ti-report-money"></i>{{\Auth::user()->priceFormat($deal->price)}}</li>
                                                        </ul>
                                                        <div class="user-group">
                                                            @foreach($deal->users as $user)
                                                                <a href="#" class="" data-original-title="{{$user->name}}" data-toggle="tooltip">
