...
@section('action-btn')
    @if(\Auth::user()->type=='company')
        <a href="{{ route('zoommeeting.calendar') }}" class="btn btn-sm btn-primary btn-icon m-1" data-bs-toggle="tooltip" title="Calendar View" >
                        ...
                        <table class="table" id="pc-dt-simple">
                            ...
                            <tbody>
                                ...
                                <td class="user-group1"> 
                                    @foreach($item->users($item->employee) as $projectUser)
                                        <!-- <a href="#" class="user-group1"> -->
                                            <img @if(!empty($projectUser->avatar)) src="{{asset('/storage/uploads/avatar/'.$projectUser->avatar)}}" @else avatar="{{(!empty($projectUser)?$projectUser->name:'')}}" @endif data-original-title="{{(!empty($projectUser)?$projectUser->name:'')}}" data-toggle="tooltip" data-original-title="{{(!empty($projectUser)?$projectUser->name:'')}}" class="">
