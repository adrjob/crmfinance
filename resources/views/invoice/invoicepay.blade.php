...
@section('action-btn')
    ...
    @if($invoice->getDue() > 0)
        <a href="#" data-bs-toggle="modal" data-bs-target="#paymentModal" class="btn btn-sm btn-primary btn-icon rounded-pill">
                        ...
                        <div class="col-12">
                            ...
                            <div class="row">
                                <div class="col-lg-6 col-md-8">
                                    ...
                                    <span class="col-sm-8"><span class="text-sm">{{ $users->invoiceNumberFormat($invoice->id) }}</span></span>
                                </div>
                            </div>
                            <div class="row mb-5">
                                <div class="col-lg-6 col-md-8">
                                    <h6 class="d-inline-block m-0 d-print-none">{{__('Invoice Date')}}</h6>
                                    <span class="col-sm-8"><span class="text-sm">{{$users->dateFormat($invoice->date_invoiced)}}</span></span>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-12">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table mb-0">
                                            <thead>
                                            <tr>
                                                <th class="px-2 bg-transparent border-top-0">{{__('Item')}}</th>
                                                <th class="px-2 bg-transparent border-top-0">{{__('Quantity')}}</th>
                                                <th class="px-2 bg-transparent border-top-0">{{__('Price')}}</th>
                                                <th class="px-2 bg-transparent border-top-0">{{__('Tax')}}</th>
                                                <th class="px-2 bg-transparent border-top-0">{{__('Discount')}}</th>
                                                <th class="px-2 bg-transparent border-top-0">{{__('Description')}}</th>
                                                <th class="px-2 bg-transparent border-top-0 text-right">{{__('Price')}}</th>
                                            ...
                                            @foreach($invoice->items as $invoiceitem)
                                                ...
                                                @endphp
                                                <tr>
                                                    <td class="px-2">{{!empty($invoiceitem->items) ? $invoiceitem->items->name : '-' }} </td>
                                                    <td class="px-2">{{$invoiceitem->quantity}} </td>
                                                    <td class="px-2">{{$users->priceFormat($invoiceitem->price)}} </td>
                                                    <td class="px-2">
                                                        <div class="col">
                                                            @if(!empty($invoiceitem->tax))
                                                                @foreach($invoiceitem->tax($invoiceitem->tax) as $tax)
                                                                    @php
                                                                        $taxPrice=App\Models\Utility::taxRate($tax->rate,$invoiceitem->price,$invoiceitem->quantity);
                                                    ...
                                                    <td class="px-2">{{$users->priceFormat($invoiceitem->discount)}} </td>
                                                    <td class="px-2">{{$invoiceitem->description}} </td>
                                                    <td class="text-right"> {{$users->priceFormat($invoiceitem->price*$invoiceitem->quantity)}}</td>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right subTotal">{{$users->priceFormat($invoice->getSubTotal())}}</td>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right subTotal">{{$users->priceFormat($invoice->getTotalDiscount())}}</td>
                                                ...
                                                @foreach($taxesData as $taxName => $taxPrice)
                                                    @if($taxName != 'No Tax')
                                                        <tr>
                                                            ...
                                                            <td class="text-right">{{ $users->priceFormat($taxPrice) }}</td>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right subTotal">{{$users->priceFormat( $invoice->getTotal())}}</td>
                                            ...
                                            <div class="row justify-content-between align-items-center">
                                                <div class="col-md-6 order-md-2 mb-4 mb-md-0">
                                                    <div class="d-flex align-items-center justify-content-md-end">
                                                        ...
                                                        <span class="h4 mb-0">{{$users->priceFormat($invoice->getTotal())}}</span>
                                                    </div>
                                                </div>
                    ...
                    <ul class="list-group list-group-flush">
                        <li class="list-group-item px-0">
                            <div class="row align-items-center">
                                ...
                                <dd class="col-sm-12"><span class="text-sm">{{$users->dateFormat($invoice->created_at)}}</span></dd>
                            </div>
                        </li>
                    </ul>
                </div>
                            ...
                            <ul class="nav nav-tabs nav-overflow profile-tab-list" role="tablist">
                                ...
                                @if(isset($payment_setting['is_stripe_enabled']) && $payment_setting['is_stripe_enabled'] == 'on')
                                    ...
                                    (isset($payment_setting['stripe_secret']) && !empty($payment_setting['stripe_secret'])))
                                        ...
                                        <li class="nav-item ml-4">
                                            <a href="#stripe-payment" class="nav-link active" data-bs-toggle="tab" role="tab" aria-selected="false">
                                                {{ __('Stripe') }}
                                            </a>
                                        </li>
                                    @endif
                                ...
                                @if(isset($payment_setting['is_paypal_enabled']) && $payment_setting['is_paypal_enabled'] == 'on')
                                    ...
                                    (isset($payment_setting['paypal_secret_key']) && !empty($payment_setting['paypal_secret_key'])))
                                        <li class="nav-item ml-4">
                                            <a data-bs-toggle="tab" href="#paypal-payment" class="nav-link" role="tab" aria-controls="paypal" aria-selected="false">{{ __('Paypal') }}</a>
                                        </li>
                                    @endif
                                @endif
                                ...
                                @if(isset($payment_setting['is_paystack_enabled']) && $payment_setting['is_paystack_enabled'] == 'on')
                                    ...
                                    (isset($payment_setting['paystack_secret_key']) && !empty($payment_setting['paystack_secret_key'])))
                                        <li class="nav-item ml-4">
                                            <a data-bs-toggle="tab" href="#paystack-payment" class="nav-link" role="tab" aria-controls="paystack" aria-selected="false">{{ __('Paystack') }}</a>
                                        </li>
                                    @endif
                                @endif
                                ...
                                @if(isset($payment_setting['is_flutterwave_enabled']) && $payment_setting['is_flutterwave_enabled'] == 'on')
                                    ...
                                    (isset($payment_setting['flutterwave_public_key']) && !empty($payment_setting['flutterwave_public_key'])))
                                        <li class="nav-item ml-4">
                                            <a data-bs-toggle="tab" href="#flutterwave-payment" class="nav-link" role="tab" aria-controls="flutterwave" aria-selected="false">{{ __('Flutterwave') }}</a>
                                        </li>
                                    @endif
                                @endif
                                ...
                                @if(isset($payment_setting['is_razorpay_enabled']) && $payment_setting['is_razorpay_enabled'] == 'on')
                                    ...
                                    (isset($payment_setting['razorpay_secret_key']) && !empty($payment_setting['razorpay_secret_key'])))
                                        <li class="nav-item ml-4">
                                            <a data-bs-toggle="tab" href="#razorpay-payment" class="nav-link" role="tab" aria-controls="razorpay" aria-selected="false">{{ __('Razorpay') }}</a>
                                        </li>
                                    @endif
                                @endif
                                ...
                                @if(isset($payment_setting['is_mercado_enabled']) && $payment_setting['is_mercado_enabled'] == 'on')
                                    @if((isset($payment_setting['mercado_access_token']) && !empty($payment_setting['mercado_access_token'])))
                                        <li class="nav-item ml-4">
                                            <a data-bs-toggle="tab" href="#mercado-payment" class="nav-link" role="tab" aria-controls="mercado" aria-selected="false">{{ __('Mercado Pago') }}</a>
                                        </li>
                                    @endif
                                @endif
                                ...
                                @if(isset($payment_setting['is_paytm_enabled']) && $payment_setting['is_paytm_enabled'] == 'on')
                                    ...
                                    (isset($payment_setting['paytm_merchant_key']) && !empty($payment_setting['paytm_merchant_key'])))
                                        <li class="nav-item ml-4">
                                            <a data-bs-toggle="tab" href="#paytm-payment" class="nav-link" role="tab" aria-controls="paytm" aria-selected="false">{{ __('Paytm') }}</a>
                                        </li>
                                    @endif
                                @endif
                                ...
                                @if(isset($payment_setting['is_mollie_enabled']) && $payment_setting['is_mollie_enabled'] == 'on')
                                    ...
                                    (isset($payment_setting['mollie_profile_id']) && !empty($payment_setting['mollie_profile_id'])))
                                        <li class="nav-item ml-4">
                                            <a data-bs-toggle="tab" href="#mollie-payment" class="nav-link" role="tab" aria-controls="mollie" aria-selected="false">{{ __('Mollie') }}</a>
                                        </li>
                                    @endif
                                @endif
                                ...
                                @if(isset($payment_setting['is_skrill_enabled']) && $payment_setting['is_skrill_enabled'] == 'on')
                                    @if((isset($payment_setting['skrill_email']) && !empty($payment_setting['skrill_email'])))
                                        <li class="nav-item ml-4">
                                            <a data-bs-toggle="tab" href="#skrill-payment" class="nav-link" role="tab" aria-controls="skrill" aria-selected="false">{{ __('Skrill') }}</a>
                                        </li>
                                    @endif
                                @endif
                                ...
                                @if(isset($payment_setting['is_coingate_enabled']) && $payment_setting['is_coingate_enabled'] == 'on')
                                    @if((isset($payment_setting['coingate_auth_token']) && !empty($payment_setting['coingate_auth_token'])))
                                        <li class="nav-item ml-4">
                                            <a data-bs-toggle="tab" href="#coingate-payment" class="nav-link" role="tab" aria-controls="coingate" aria-selected="false">{{ __('CoinGate') }}</a>
                                        </li>
                                    @endif
                                @endif
                                ...
                                @if(isset($payment_setting['is_paymentwall_enabled']) && $payment_setting['is_paymentwall_enabled'] == 'on')
                                    ...
                                    (isset($payment_setting['paymentwall_private_key']) && !empty($payment_setting['paymentwall_private_key'])))
                                        <li class="nav-item">
                                            <a data-bs-toggle="tab" href="#paymentwall-payment" class="nav-link" role="tab" aria-controls="paymentwall" aria-selected="false" >{{__('PaymentWall')}}</a>
                                        </li>
                                    @endif
                                @endif
                            </ul>
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="form-group col-md-12">
                                                            ...
                                                            <div class="form-icon-addon">
                                                                <span>{{isset($payment_setting['currency_symbol'])?$payment_setting['currency_symbol']:'$'}}</span>
                                                                <input class="form-control" required="required" min="0" name="amount" type="number" value="{{$invoice->getDue()}}" min="0" step="0.01" max="{{$invoice->getDue()}}" id="amount">
                                                                <input type="hidden" value="{{\Illuminate\Support\Facades\Crypt::encrypt($invoice->id)}}" name="invoice_id">
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="col-12 form-group mt-3 text-right">
                                            ...
                                            @if((isset($payment_setting['razorpay_public_key']) && !empty($payment_setting['razorpay_public_key'])) && 
                                                (isset($payment_setting['razorpay_secret_key']) && !empty($payment_setting['razorpay_secret_key'])))
                                                    ...
                                                    <form method="post" action="{{route('invoice.pay.with.razorpay')}}" class="require-validation" id="razorpay-payment-form">
                                                        ...
                                                        <div class="col-12 form-group mt-3 text-right">
                                                            <input type="button" value="{{__('Make Payment')}}" class="btn btn-sm btn-primary rounded-pill" id="pay_with_razorpay">
                                                        </div>
                                                    </form>
                        ...
                        var x = getpaidSetup({
                            ...
                            callback: function (response) {
                                ...
                                if(response.tx.chargeResponseCode == "00" || response.tx.chargeResponseCode == "0") {
                                    window.location.href = "{{url('/invoice/flaterwave')}}/"+txref+"/{{encrypt($invoice->id)}}/"+res.total_price;
                                }else{
