...
@section('action-btn')
   @if(\Auth::user()->type=='company')
        @if($pipeline)
            <div class="btn-group">
                <button class="btn btn-sm btn-primary btn-icon m-1 dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-haspopup="true"
                    aria-expanded="false">
                    {{$pipeline->name}}
                </button>
    ...
    @if(\Auth::user()->type=='company')
        ...
        data-bs-target="#exampleModal" data-url="{{ route('lead.create') }}"
        data-bs-whatever="{{__('Create New Lead')}}"
                ...
                @php $leads = $lead_stage->lead() @endphp
                    <div class="col">
                        <div class="card">
                            ...
                            <div class="card-body kanban-box" id="kanban-blacklist-{{$lead_stage->id}}" data-id="{{$lead_stage->id}}">
                                        ...
                                        <div class="pt-3 ps-3">
                                            ...
                                            <div class="card-header border-0 pb-0 position-relative">
                                                <h5>
                                                    ...
                                                    data-bs-toggle="tooltip"  title data-bs-original-title="{{__('Lead Detail')}}" >{{$lead->name}}</a></h5>
                                                    <div class="card-header-right">
                                                        <div class="btn-group card-option">
                                                            ...
                                                            <div class="dropdown-menu dropdown-menu-end">
                                                                ...
                                                                @else
                                                                    @if(\Auth::user()->type=='company')
                                                                        <a href="#!" class="dropdown-item" data-size="lg" data-url="{{ route('lead.edit',$lead->id) }}"
                                                                    ...
                                                                    @if(\Auth::user()->type=='company')
                                                                    <a href="#!" class="dropdown-item" data-size="lg"  data-url="{{ route('lead.label',$lead->id) }}"
                                                                        data-bs-toggle="modal" data-bs-target="#exampleModal"  data-bs-whatever="{{__('Add Label')}}">
                                            ...
                                            <div class="card-body">
                                                <p class="text-muted text-sm" data-bs-toggle="tooltip" data-bs-original-title="{{ __('Description') }}">{{ $lead->subject }}</p>                                                <div class="d-flex align-items-center justify-content-between">
                                                    <ul class="list-inline mb-0">
                                                        <li class="list-inline-item d-inline-flex align-items-center"><i
                                                                class="f-16 text-primary ti ti-message-2"></i>{{\Auth::user()->dateFormat($lead->date)}}</li>
                                                    </ul>
