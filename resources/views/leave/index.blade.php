...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="collapse {{isset($_GET['employee'])?'show':''}}" id="collapseExample">
                <div class="card card-body">
                    ...
                    <div class="row filter-css">
                        ...
                        <!-- <div class="col-auto my-1">
                            <button type="submit" class="action-btn bg-info ms-1 mx-0 btn btn-sm d-flex align-items-center" data-toggle="tooltip" data-title="{{__('Apply')}}"><i class="ti ti-search text-white"></i></button>
                        </div> -->
                        ...
                        <!-- <div class="col-auto my-1">
                            <a href="{{route('leave.index')}}" data-toggle="tooltip" data-title="{{__('Reset')}}" class="action-btn bg-danger ms-1 mx-1 btn btn-sm d-flex align-items-center"><i class="ti ti-trash-off text-white"></i></a>
                        </div> -->
                    </div>
                    {{ Form::close() }}
                </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($leave->status=="Pending")
                                            <div class="badge fix_badge bg-warning p-2 px-3 rounded">{{ $leave->status }}</div>
                                        @elseif($leave->status=="Approve")
                                            <div class="badge fix_badge bg-success p-2 px-3 rounded">{{ $leave->status }}</div>
                                        @else($leave->status=="Reject")
                                            <div class="badge fix_badge bg-danger p-2 px-3 rounded">{{ $leave->status }}</div>
                                        @endif
                                    </td>
                                    @if(\Auth::user()->type=='company' || \Auth::user()->type=='employee')
                                        <div class="row ">
                                            <td class="">
                                                @if(\Auth::user()->type=='company')
                                                    <div class="action-btn bg-warning ms-2">
                                                        <a href="#" class="mx-3 btn btn-sm d-inline-flex align-items-center" data-bs-toggle="modal"
                                                            data-bs-target="#exampleModal" data-url="{{ route('leave.action',$leave->id) }}"
                                                            data-bs-whatever="{{__('View Leave')}}"> <span class="text-white"> <i
                                                ...
                                                <div class="action-btn bg-info ms-2">
                                                    <a href="#" class="mx-3 btn btn-sm d-inline-flex align-items-center" data-bs-toggle="modal"
                                                        data-bs-target="#exampleModal" data-url="{{ route('leave.edit',$leave->id) }}"
                                                        data-bs-whatever="{{__('Edit Leave')}}" data-size="lg"> <span class="text-white"> <i
                                                                class="ti ti-edit" data-bs-toggle="tooltip" data-bs-original-title="{{ __('Edit') }}"></i></span>
                                                    </a>
                                                </div>
