...
@section('action-btn')
    @if(\Auth::user()->type=='company')
        ...
        <a href="{{route('creditnote.export')}}" class="btn btn-sm btn-primary btn-icon m-1" data-bs-original-title="{{__('Export invoice CSV file')}}" data-bs-toggle="tooltip">
            <i class="ti ti-file-export"></i>
        </a>

        <a href="#" data-size="lg" data-url="{{ route('creditNote.create') }}"  data-bs-toggle="modal"
        data-bs-target="#exampleModal" data-bs-whatever="{{ __('Create Payment') }}" class="btn btn-sm btn-primary btn-icon m-1">
                                        ...
                                        <tr>
                                            <td>
                                                <div class="media-body">
                                                    <a href="{{route('invoice.show',\Crypt::encrypt($invoice->id))}}" class="name h6 mb-0 text-sm"> {{ AUth::user()->invoiceNumberFormat($invoice->invoice_id) }}</a>
                                                </div>
                                            ...
                                            <td class="table-actions text-right">
                                                ...
                                                <div class="action-btn bg-info ms-2">
                                                    <a href="#" data-size="lg" data-url="{{ route('creditNote.edit',$creditNote->id) }}" data-bs-toggle="modal"
                                                        data-bs-target="#exampleModal"  data-bs-whatever="{{__('Edit Credit Note')}}"
                                                        class="mx-3 btn btn-sm d-inline-flex align-items-center">
