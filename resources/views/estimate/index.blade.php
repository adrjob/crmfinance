...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="collapse {{isset($_GET['start_date'])?'show':''}}" id="collapseExample">
                ...
                <div class="card hover-shadow-lg">
                    <div class="card-header border-0">
                        <div class="row align-items-center">
                            <div class="col-10">
                                <h6 class="mb-0">
                                    <a href="{{ route('estimate.show',\Crypt::encrypt($estimate->id)) }}">{{\Auth::user()->estimateNumberFormat($estimate->estimate)}}</a>
                                </h6>
                            </div>
                                ...
                                <div class="actions">
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                            @if(\Auth::user()->type=='company')
                                                <a href="{{ route('estimate.edit',\Crypt::encrypt($estimate->id)) }}" class="dropdown-item">
                                                   <i class="ti ti-edit"></i> {{__('Edit')}}
                                                </a>

                                                {!! Form::open(['method' => 'DELETE', 'route' => ['estimate.destroy', $estimate->id]]) !!}
                                            ...
                                            @if(\Auth::user()->type=='company' || \Auth::user()->type=='client')
                                                <a href="{{ route('estimate.show',\Crypt::encrypt($estimate->id)) }}" class="dropdown-item">
                    ...
                    <div class="card-body pt-0">
                        <div class="p-3 border border-dashed">
                            ...
                            @if($estimate->status == 0)
                                <span class="badge bg-primary p-2 px-3 rounded ">{{ __(\App\Models\Estimate::$statues[$estimate->status]) }}</span>
                            @elseif($estimate->status == 1)
                                <span class="badge bg-info p-2 px-3 rounded">{{ __(\App\Models\Estimate::$statues[$estimate->status]) }}</span>
                            @elseif($estimate->status == 2)
                                <span class="badge bg-success p-2 px-3 rounded">{{ __(\App\Models\Estimate::$statues[$estimate->status]) }}</span>
                            @elseif($estimate->status == 3)
                                <span class="badge bg-warning p-2 px-3 rounded">{{ __(\App\Models\Estimate::$statues[$estimate->status]) }}</span>
                            @elseif($estimate->status == 4)
                                <span class="badge bg-danger p-2 px-3 rounded">{{ __(\App\Models\Estimate::$statues[$estimate->status]) }}</span>
                            @endif
                        ...
                        @if(\Auth::user()->type != 'client')
                            ...
                            <div class="media mt-4 align-items-center">
                                <img @if(!empty($client->avatar)) src="{{$profile.'/'.$client->avatar}}" @else avatar="{{$estimate->clients->name}}" @endif class="avatar rounded-circle avatar-custom" data-toggle="tooltip" data-original-title="{{__('Client')}}">
